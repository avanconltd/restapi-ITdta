...
class ADXManager:
    ...
    def ingest_data(self, table_name: str, data: List[Dict]) -> bool:
        ...
        try:
            ...
            logging.info(f"Total rows to ingest: {len(data)}")

            if data:
                first_100 = data[:100]
                logging.info(f"First 100 rows of data: {json.dumps(first_100, default=str, indent=2)}")
            else:
            ...
            logging.info(f"Successfully queued {len(data)} records for ingestion to {table_name}")
            return True
